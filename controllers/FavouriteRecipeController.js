...
export const removeFromFavourite = async (req, res) => {
    try {
        ...
        const isMatch = await FavouriteRecipe.findOne({ recipe: recipeId, user: userId });
        ...
        const result = await FavouriteRecipe.deleteOne({ recipe: recipeId, user: userId }); 
...
export const addToFavourite = async (req, res) => {
    try {
        ...
        const isMatch = await FavouriteRecipe.findOne({ recipe: recipe, user: user });
        if (isMatch) {
